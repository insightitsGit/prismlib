...
- Full PrismCache (single-node), PrismDriver (two-node WAL), ClusterCache, AlertManager
- Blue/Green/Orange failover (all code included)
- All 7 CHORUS frame types
- Apache 2.0 — use in commercial products, no attribution required
- Community support via GitHub Issues
- `pip install "prismlib[all]"` — one command, everything included

### ChorusMesh (coming soon — separate package)

ChorusMesh is the cluster orchestration companion to PrismLib. It will be a
separate PyPI package (`pip install chorusmesh`) with a 30-day free trial then
a subscription model. Planned tiers:

| Tier | Nodes | Price | Includes |
|------|-------|-------|----------|
| Developer | Up to 3 | Free (30 days) then $29/mo | ClusterCache, failover, AlertManager |
| Team | Up to 10 | $149/mo | + Raft consensus, message broker adapters |
| Business | Up to 50 | $499/mo | + multi-region routing, SLA 99.9% |
| Enterprise | Unlimited | Contact us | + air-gap, compliance docs, dedicated Slack |

> These are indicative prices for the product page. Adjust before publishing.
> The user will confirm final numbers.

### PrismLib Enterprise

Enterprise agreements cover the full PrismLib stack (PrismCache + PrismDriver +
ClusterCache + CHORUS Fabric) with commercial support. What enterprise includes:

- **On-premises / air-gapped deployment** — hardened Docker images, SOC 2 docs, no phone-home
...
No public pricing. Every deployment is different. Contact: insightits.info@gmail.com
